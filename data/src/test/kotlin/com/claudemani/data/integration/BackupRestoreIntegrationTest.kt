...
import com.claudemani.database.CLAUDEMANI_SCHEMA_VERSION
...
import com.claudemani.domain.usecase.CreateBackupUseCase
import com.claudemani.domain.usecase.CreateConceptCommand
import com.claudemani.domain.usecase.CreateConceptUseCase
import com.claudemani.domain.usecase.RestoreBackupUseCase
import com.claudemani.domain.usecase.RestoreResult
...
class BackupRestoreIntegrationTest {
    ...
    fun `full backup from one real database restores completely into an empty one`() = runTest {
        ...
        val backup = createBackupUseCase(source)(BackupType.FULL, CLAUDEMANI_SCHEMA_VERSION, Instant.now())
        ...
        val result = restoreBackupUseCase(target)(
            data = backup,
            currentSchemaVersion = CLAUDEMANI_SCHEMA_VERSION,
            now = Instant.now(),
    ...
    fun `restoring the same backup twice is idempotent - second pass merges instead of duplicating`() = runTest {
        ...
        val first = restore(backup, CLAUDEMANI_SCHEMA_VERSION, Instant.now(), { true }, { true }) as RestoreResult.Success
        val second = restore(backup, CLAUDEMANI_SCHEMA_VERSION, Instant.now(), { true }, { true }) as RestoreResult.Success
