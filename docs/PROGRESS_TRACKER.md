...
## فاز ۳۸ (بعد از v1.3.0): مورد ۶ — Backlog سند (تصمیم آگاهانه با اجازه کاربر)

کاربر صراحتاً درخواست داد خودم تصمیم بگیرم و به‌عنوان «تصمیم آگاهانه» مستند کنم. از ۴ زیرمجموعه Backlog:

### پیاده‌سازی‌شده: Quiz Difficulty + Category-aware Distractor Selection
- `QuizDifficulty` enum جدید (EASY/MEDIUM/HARD) در `domain/model/Enums.kt` — کاملاً مستقل از `VocabularyDifficulty`، فقط پارامتر زمان-تولید سؤال، هیچ‌جا Persist نمی‌شود.
- `GenerateQuizQuestionUseCase`: Pool اصلی (۳ مرحله بر اساس Vocabulary Difficulty، همون منطق FROZEN قبلی) **دست‌نخورده** ماند. `quizDifficulty` فقط یک مرحله ری‌رنکینگ *بعدی* روی همون Pool است: HARD همون Category کانسپت هدف را در اولویت می‌گذارد، EASY دسته‌های متفاوت را، MEDIUM (پیش‌فرض) هیچ تغییری نمی‌دهد — یعنی رفتار پیش‌فرض قبلی این UseCase بایت‌به‌بایت حفظ شده و هیچ تست قبلی نشکست.
- **تصمیم صریح مستندشده:** خودِ متن Backlog برای MEDIUM از «Category یا Subcategory مشابه» حرف می‌زند، ولی مدل داده اصلاً مفهوم Subcategory یا شباهت بین Categoryها ندارد (Category تخت است). به‌جای اختراع یک معیار شباهت جعلی، MEDIUM = «بدون ری‌رنکینگ» تعریف شد — یک تفسیر صادقانه، نه یک حدس.
- Settings: انتخابگر ۳-حالته (آسان/متوسط/سخت) که در `AppSetting` ذخیره می‌شود؛ `ReviewViewModel` یک‌بار در شروع Session می‌خواندش.
- تست: ۵ تست جدید در `GenerateQuizQuestionUseCaseTest` (شامل حالت «به‌اندازه کافی هم‌دسته نیست → از بقیه Pool پر شود» و «Concept بدون Category هرگز ری‌رنک نمی‌شود»).

### پیاده‌سازی‌شده: رمزنگاری Backup (Descriptions §16.2)
- **تصمیم کلیدی:** سند دو گزینه کلید می‌دهد: Android Keystore یا PIN کاربر. **PIN انتخاب شد، نه Keystore** — چون کلید Keystore به یک نصب خاص روی یک دستگاه خاص گره می‌خورد و معمولاً با حذف/نصب مجدد اپ از بین می‌رود؛ یعنی دقیقاً همون رویدادی (نصب مجدد، گوشی جدید) که Backup قرار است در برابرش محافظت کند، خودِ رمزنگاری Keystore-محور را غیرقابل بازگشایی می‌کرد. این یک تناقض داخلی در ایده بود که با انتخاب PIN حل شد.
- `data/backup/BackupEncryption.kt`: AES/GCM با کلید مشتق از PBKDF2WithHmacSHA256(PIN, Salt تصادفی). کاملاً `javax.crypto` خالص — بدون AndroidKeyStore، بدون Context — پس با تست Unit ساده (بدون Robolectric) قابل تست است. یک Magic Header (۵ بایت) در ابتدای فایل رمزنگاری‌شده باعث می‌شود Restore بدون توجه به وضعیت فعلی سوییچ تنظیمات، خودش تشخیص دهد فایل رمزنگاری‌شده هست یا نه.
- `SettingsUseCases.kt`: `Get/SetBackupEncryptionEnabledUseCase` — کلید `backup_encryption_enabled` از قبل (فازهای خیلی قبل) در `SettingKeys` رزرو شده بود ولی هیچ‌وقت استفاده نشده بود.
- `BackupViewModel`: امضای `exportBackup`/`restoreBackup` از `String` به `ByteArray` تغییر کرد (چون خروجی رمزنگاری‌شده دیگر متن UTF-8 معتبر نیست).
- UI: یک Switch «رمزنگاری Backup» در تنظیمات + یک Dialog ورود PIN (هم برای Export هم Restore) در `SettingsScreen`.
- تست: ۶ تست خالص در `BackupEncryptionTest` (Round-trip، PIN غلط، تشخیص فایل رمزنگاری‌نشده، غیرتکراری‌بودن Ciphertext، داده خراب).
- **محدودیت صادقانه:** این رمزنگاری هرگز روی دستگاه واقعی تست نشده (فقط منطق ریاضی/رمزنگاری آن، نه فرآیند واقعی SAF+نوشتن فایل باینری). ریسک کمی دارد ولی الگوی AES/GCM+PBKDF2 استاندارد و رایج است.

### عمداً پیاده نشد: چند LanguagePair هم‌زمان
طبق تصمیم اعلام‌شده قبل از شروع این فاز: این مورد نیازمند Migration واقعی Schema (افزودن `languagePairId` به `LearningState`، طبق توضیح صریح خودِ Descriptions §19.1) است — یعنی یک تغییر دیتابیس واقعی که طبق قانون خودم («قبل از تغییر دیتابیس، جدا اطلاع بده») باید مستقل و با توضیح روشن انجام شود، نه قاطی کارهای دیگر این فاز. اگر کاربر بخواهد، این باید فاز جداگانه‌ی خودش باشد.

## فاز ۳۹ (بعد از v1.4.0): سومین اجرای واقعی CI — یک باگ کامپایل جدید

`v1.4.0` (فاز ۳۸، Quiz Difficulty + رمزنگاری Backup) اولین بار واقعاً روی CI اجرا شد. نتیجه:

```
GenerateQuizQuestionUseCase.kt:118:69 The feature "break continue in inline lambdas" is experimental and should be enabled explicitly
```
